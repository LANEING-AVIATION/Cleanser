...
def set_takeoff_mode(master, mode_id):
    try:
        print(f"尝试切换到模式: {mode_id}")
        master.mav.set_mode_send(
            master.target_system,  # 目标系统 ID
            mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED,  # 基本模式标志
            mode_id  # 自定义模式 ID
        )
...
def arm_takeoff(master):
    try:
        ...
        master.mav.command_long_send(
            master.target_system,  # 目标系统 ID
            master.target_component,  # 目标组件 ID
            mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,  # 解锁电机命令
            0,  # 确认标志
            1,  # 1 = 解锁, 0 = 锁定
            0, 0, 0, 0, 0, 0  # 其他参数留空
        )
        ...
        # 等待解锁反馈
        time.sleep(1)
        arm_feedback = master.recv_match(type='COMMAND_ACK', blocking=True, timeout=5)
        if arm_feedback:
            print(f"解锁反馈: {arm_feedback}")
            if arm_feedback.result != 0:
                print(f"解锁失败，错误代码: {arm_feedback.result}")
        else:
            print("没有收到解锁反馈信息")
    except Exception as e:
        print(f"解锁失败: {str(e)}")

def takeoff_sequence(master):
    # 切换到稳定模式
    print("切换到稳定模式...")
    set_takeoff_mode(master, mode_id=0)  # 模式 ID 为 0 表示 STABILIZE 模式
    ...
    # 切换到任务模式
    print("切换到任务模式...")
    set_takeoff_mode(master, mode_id=4)  # 模式 ID 为 4 表示 MISSION 模式
